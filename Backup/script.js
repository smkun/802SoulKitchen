...
const renderEvents = (events) => {
    ...
    activeEvents.forEach((event, index) => {
        ...
        eventElement.innerHTML = `
            <h3 class="text-2xl font-bold mb-4 gradient-text">${event.name}</h3>
            ...
            <p class="mt-6 text-lg text-brand-white leading-relaxed">${event.description}</p>
        `;
...
const renderMenu = (menuItems) => {
    ...
    categories.forEach(category => {
        if (categorizedItems[category] && categorizedItems[category].length > 0) {
            ...
            menuHTML += `<h3 class="text-xl font-bold mb-2 border-b border-brand-orange/30 pb-2 text-brand-white">${category}</h3>`;
